...
def sample_diffusion_ligand(model, data, num_samples, batch_size=15, device='cuda:2',
                            num_steps=None, pos_only=False, center_pos_mode='protein',
                            sample_num_atoms='prior',fix_num=False,return_attention=False,size_sampler=None,pocket_size=None):
    ...
    for i in tqdm(range(num_batch),ascii=True):
        ...
        with torch.no_grad():
            ...
            if sample_num_atoms == 'prior':
                ...
                if fix_num:
                    ligand_num_atoms = [data['ligand_element_length'][idx] for idx in range(n_data)]
                else:
                    ligand_num_atoms = [atom_num.sample_atom_num(pocket_size[idx],size_sampler).astype(int) for idx in range(n_data)]

                batch_ligand = torch.tensor(sum([[idx]* ligand_num_atoms[idx] for idx in range(n_data)],[])).to(device)
            ...
            # init ligand pos
            center_pos = scatter_mean(batch['protein_pos'], batch_protein, dim=0).to(device)
            batch_center_pos = center_pos[batch_ligand]
            # pos_seed = torch.randn_like(batch_center_pos[:int(batch_center_pos.shape[0]//6),:]).repeat(6,1)
            init_ligand_pos = batch_center_pos + torch.randn_like(batch_center_pos)
            # init_ligand_pos = batch_center_pos + pos_seed

            # init_ligand_pos = batch_center_pos + torch.randn_like(batch_center_pos[:int(ligand_num_atoms[0]/10),:]).repeat(len(center_pos),1)
            # init ligand v
            if pos_only:
                init_ligand_v = batch['ligand_atom_feature_full'].to(device)
            else:
                # uniform_logits = torch.zeros(len(batch_ligand)//6, model.num_classes).to(device)
                ...
                # init_ligand_v = log_sample_categorical(uniform_logits).repeat(6)

                # init_ligand_v = log_sample_categorical(uniform_logits[:int(ligand_num_atoms[0]/10),:]).repeat(len(center_pos))
            batch['interaction'] = batch['interaction'].long()
            r = model.sample_diffusion(
                protein_pos=batch['protein_pos'].to(device),
                protein_v=batch['protein_atom_feature'].to(device),
        ...
        current_i += n_data
    return all_pred_pos, all_pred_v, all_pred_pos_traj, all_pred_v_traj, all_pred_v0_traj, all_pred_vt_traj, time_list,r['atts']
