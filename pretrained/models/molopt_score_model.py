...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        super().__init__()
        self.config = config
        self.prompt_emb = nn.Embedding(4,56)
        # variance schedule
    ...
    def forward(self, protein_pos, protein_v, batch_protein,init_ligand_pos, init_ligand_v, batch_ligand,
            prompt,interaction, time_step=None, return_all=False, fix_x=False):
...
#mask_ligand 总节点数目,true表示是ligand节点
        ...
        outputs = self.refine_net(h_all, pos_all, mask_ligand, batch_all, prompt,interaction,return_all=return_all, fix_x=fix_x)
    ...
    def get_diffusion_loss(
        ...
        interaction=None,time_step=None):
        ...
        loss = 10*loss_pos + loss_v * self.loss_v_weight
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand,prompt,interaction,
