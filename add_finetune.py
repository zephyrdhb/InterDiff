...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        out = self.h_to_out(out).to(dtype)
...
class wrap_atte(nn.Module):
    ...
    def forward(self, h, x, edge_type, edge_index, mask_ligand,protein_len,ligand_len,dist_feat, interaction,e_w, fix_x,return_attention=False):
        ...
        h,x = self.model(h, x, edge_type, edge_index, mask_ligand,protein_len,ligand_len,dist_feat,interaction=interaction, e_w=e_w, fix_x=fix_x)

        h_protein, h_ligand = h[~mask_ligand], h[mask_ligand]
        x_ligand, x_protein = x[mask_ligand],x[~mask_ligand]

        batch_dis = list(map(lambda x:x[0][:,None,:]-x[1],zip(x_protein.split(protein_len,0),x_ligand.split(ligand_len,0))))
        ...
        h[mask_ligand] = h[mask_ligand] + input
        x[mask_ligand] = x[mask_ligand] + batch_dis.mean(-1)
        if not return_attention:
            return h, x
        else:
            return h, x, att
