...
class BaseX2HAttLayer(nn.Module):
    ...
    def forward(self, h, r_feat, edge_feat, edge_index,pro_prompt=None,lig_prompt=None,e_w=None):
        N = h.size(0)#num_nodes
        src, dst = edge_index#2*num_edges
        hi, hj = h[dst], h[src]#num_edges*128
...
class BaseH2XAttLayer(nn.Module):
    ...
    def forward(self, h, rel_x, r_feat, edge_feat, edge_index,pro_prompt=None,lig_prompt=None,e_w=None):
        N = h.size(0)
        src, dst = edge_index
        hi, hj = h[dst], h[src]
        ...
        # multi-head attention
        # decide inputs of k_func and v_func
        kv_input = torch.cat([r_feat, hi, hj], -1)#num_edges*336 r_feat
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, pro_prompt,lig_prompt,e_w=None, fix_x=False):
        ...
        for i in range(self.num_x2h):#1
            ...
            dist_feat = outer_product(edge_attr, dist_feat)#edge_attr 4种类型的边,l-l,p-l,l-p,p-p
            h_out = self.x2h_layers[i](h_in, dist_feat, edge_feat, edge_index, pro_prompt,lig_prompt,e_w=e_w)
            h_out = h_in + h_out
            h_in = h_out
        x2h_out = h_in
        ...
        for i in range(self.num_h2x):#1
            ...
            delta_x = self.h2x_layers[i](new_h, rel_x, dist_feat, edge_feat, edge_index, pro_prompt,lig_prompt,e_w=e_w)
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def forward(self, h, x, mask_ligand, batch, pro_prompt,lig_prompt,return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):#9 layers
                h, x = layer(h, x, edge_type, edge_index, mask_ligand, pro_prompt,lig_prompt,e_w=e_w, fix_x=fix_x)
            all_x.append(x)
            all_h.append(h)
