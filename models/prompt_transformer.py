...
class BaseX2HAttLayer(nn.Module):
    ...
    def forward(self, h, r_feat, edge_feat, edge_index, e_w=None):
        ...
        k = self.hk_func(kv_input).view(-1, self.n_heads,
                                        self.output_dim // self.n_heads)  # num_edges*16*8
        # compute v
        ...
        q = self.hq_func(h).view(-1, self.n_heads,
                                 self.output_dim // self.n_heads)  # num_nodes*16*8
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    def __init__(self, hidden_dim, n_heads, num_r_gaussian, edge_feat_dim, act_fn='mish', norm=True,
        ...
        self.distance_expansion = GaussianSmearing(self.r_min, self.r_max,
    ...
    def forward(self, h, x, edge_attr, edge_index, mask_ligand, dist_feat, interaction=None,
        ...
        interaction_init = torch.zeros_like(h_in)
        interaction_init[~mask_ligand] = interaction
        h_in = h_in + interaction_init
...
class PromptTransformer(nn.Module):
    ...
    def _build_share_blocks(self):
        ...
        for l_idx in range(self.num_layers):
            layer = AttentionLayerO2TwoUpdateNodeGeneral(
                ...
                act_fn=self.act_fn,
                norm=self.norm,
                num_x2h=self.num_x2h, num_h2x=self.num_h2x, r_max=self.r_max,
                num_node_types=self.num_node_types,
                ew_net_type=self.ew_net_type, x2h_out_fc=self.x2h_out_fc,
                sync_twoup=self.sync_twoup,
            )
            base_block.append(layer)
        return nn.ModuleList(base_block)

    def get_edges(self, x, batch_mask, norm_constant=1):
    ...
    def forward(self, h, x, mask_ligand, batch, protein_len, ligand_len, interaction,
                return_all=False, fix_x=False, return_attention=False):
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):  # 9 layers
                if not return_attention:
                    h, x = layer(h, x, None, edge_index, mask_ligand, protein_len, ligand_len,
                                 dist_feat, interaction, e_w=e_w, fix_x=fix_x)
                else:
                    h, x, att = layer(h, x, None, edge_index, mask_ligand, protein_len, ligand_len,
                                      dist_feat, interaction, e_w=e_w, fix_x=fix_x,
