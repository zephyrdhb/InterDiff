...
if __name__ == '__main__':
    ...
    k = 0
    db = lmdb.open(
        './final_input_filter.lmdb',
        map_size=12*(1024*1024*1024),   #设置合适大小
        create=False,
        subdir=False,
        readonly=True,
        lock=False,
    ...
    split = torch.load('prompt_split.pt')
    train_set = {k: Subset(keys, indices=v) for k, v in split.items()}['train']

    test_set = Path('test_set').glob('*/*.pdb')
    test_files = [str(i).split('/')[-1] for i in test_set if 'clean' not in str(i)]
    for j in tqdm(train_set.dataset,total=len(train_set.dataset)):
        data = pickle.loads(db.begin().get(j))
        if data['protein_filename'].split('/')[-1][:10]+'.pdb' in test_files:
            k = k + 1
            print('found {} in test set'.format(k))
            test_files.remove(data['protein_filename'].split('/')[-1][:10]+'.pdb')
    print(test_files)
    """
    # Load configs
    ...
    dict_buffer = torch.load('./logs_diffusion/training_2023_08_15__08_52_52/checkpoints/5280.pt',map_location=lambda storage, loc:storage.cuda(1))
    re_dict = od()
    for k,v in dict_buffer['model'].items():
        re_dict[k[10:]] = v
    model.load_state_dict(re_dict, strict=False)

    mark_embedding_as_fix(model)#是否固定embedding需要实验
