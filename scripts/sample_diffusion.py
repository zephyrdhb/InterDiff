...
def unbatch_v_traj(ligand_v_traj, n_data, ligand_cum_atoms):
    ...
    all_step_v = [np.stack(step_v) for step_v in
                  all_step_v]  # num_samples * [num_steps, num_atoms_i]
    return all_step_v


def sample_diffusion_ligand(model, data, num_samples,  # interaction,  # protein_len,  # ligand_len,
                            batch_size=16, device='cuda:0', num_steps=None, pos_only=False,
        ...
        with torch.no_grad():
            ...
            if sample_num_atoms == 'prior':
                ...
                ligand_num_atoms = [atom_num.sample_atom_num(pocket_size).astype(int) for _ in
                                    range(n_data)]
                batch_ligand = torch.repeat_interleave(torch.arange(n_data),
                                                       torch.tensor(ligand_num_atoms)).to(device)
            ...
            r = model.sample_diffusion(
                ...
                protein_len=[len(batch.protein_element) // n_data] * n_data,  # added by duhuabin
                ligand_len=ligand_num_atoms,  # added by duhuabin
            )
            ...
            all_pred_v += [ligand_v_array[ligand_cum_atoms[k]:ligand_cum_atoms[k + 1]] for k in
                           range(n_data)]
