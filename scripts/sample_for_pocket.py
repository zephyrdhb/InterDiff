...
from models.new_score_model import ScorePosNet3D  # modified by duhuabin
...
def pdb_to_pocket_data(pdb_path):
    pocket_dict = PDBProtein(pdb_path).to_dict_atom_()
...
if __name__ == '__main__':
    ...
    model = ScorePosNet3D(
        ...
        # use_four_emb=True,
        device=args.device  # added by duhuabin
    ).to(args.device)
    model = BottleneckModel(model).to(args.device)  # added by duhuabin

    new_state_dict = {}
    for key, value in ckpt['model'].items():
        if key.startswith('_orig_mod'):
            key = key.replace('_orig_mod.', '')  # 移除 "_orig_mod." 前缀
        new_state_dict[key] = value

    model.load_state_dict(new_state_dict, strict=False if 'train_config' in config.model else True)
    logger.info(f'Successfully load the model! {config.model.checkpoint}')

    # Load pocket(original)
    data = pdb_to_pocket_data(args.pdb_path)
    data = transform(data)

    # add prompts
    # print(data.protein_res_ids)
    prompts = [(104, 2)]
    data['interaction'] = torch.zeros_like(data['protein_element'])
    for i, v in prompts:
        index = torch.nonzero(data.protein_res_id_orig == i).squeeze()
        data['interaction'][index] = v
