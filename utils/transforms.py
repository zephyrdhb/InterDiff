...
class FeaturizeProteinAtom(object):
    ...
    def __init__(self):
        super().__init__()
        self.atomic_numbers = torch.LongTensor([6, 7, 8, 16, ])  # C, N, O, S : : remove H 1, Se 34
        self.max_num_aa = 20
...
class FeaturizeLigandAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        x = [get_index(e, h, a, self.mode) for e, h, a in
             zip(element_list, hybridization_list, aromatic_list)]
        x = torch.tensor(x)
...
class FeaturizeLigandBond(object):
    ...
    def __call__(self, data: ProteinLigandData):
        data.ligand_bond_feature = F.one_hot(data.ligand_bond_type - 1,
                                             num_classes=len(utils_data.BOND_TYPES))
        return data
