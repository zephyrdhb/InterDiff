...
class PDBProtein(object):
    ...
    def _parse(self):
        ...
        for atom in self._enum_formatted_atom_lines():
            ...
            if chain_res_id not in residues_tmp:
                residues_tmp[chain_res_id] = {
                    'orig_id': atom['res_id'],  # added by dhb
                    'name': atom['res_name'],
                    'atoms': [next_ptr],
                    'chain': atom['chain'],
        ...
        for idx, residue in enumerate(self.residues):
            coords, backbone, atom_name = [], [], []
            element, atom_to_aa_type = [], []
            is_hydrogen = []
            for atom_idx in residue['atoms']:
                coords.append(self.pos[atom_idx])
                element.append(self.element[atom_idx])
                backbone.append(self.is_backbone[atom_idx])
                atom_to_aa_type.append(self.atom_to_aa_type[atom_idx])
                atom_name.append(self.atom_name[atom_idx])
                is_hydrogen.append(self.element[atom_idx] != 1)
            residue['coords'] = np.array(coords)
            residue['res_id'] = idx
            residue['element'] = np.array(element)
            residue['backbone'] = backbone
            residue['atom_to_aa_type'] = np.array(atom_to_aa_type)
            residue['atom_name'] = atom_name
            residue['is_hydrogen'] = np.array(is_hydrogen)
    ...
    def to_dict_atom_(self):
        hydrogen = np.hstack([i['is_hydrogen'] for i in self.residues])
        return {
            'element': np.hstack([i['element'] for i in self.residues], dtype=np.int64)[hydrogen],
            'pos': np.vstack([i['coords'] for i in self.residues], dtype=np.float32)[hydrogen],
            'is_backbone': np.hstack([i['backbone'] for i in self.residues], dtype=np.bool_)[hydrogen],
            'atom_name': np.hstack([i['atom_name'] for i in self.residues])[hydrogen].tolist(),
            'atom_to_aa_type': np.hstack([i['atom_to_aa_type'] for i in self.residues], dtype=np.int64)[hydrogen],
            'res_id': np.array(sum([[idx] * len(i['element']) for idx, i in enumerate(self.residues)], []), dtype=np.int64)[hydrogen],
            'res_id_orig': np.array(sum([[i['orig_id']] * len(i['element']) for idx, i in enumerate(self.residues)], []), dtype=np.int64)[hydrogen],
        }
    ...
    def query_residues_ligand_(self, ligand, radius):
        selected = []
        sel_idx = set()
        for i, residue in enumerate(self.residues):
    ...
    @staticmethod
    def residues_to_dict_atom_(residues):
        hydrogen = np.hstack([i['is_hydrogen'] for i in residues])
        return {
            'element': np.hstack([i['element'] for i in residues], dtype=np.int64)[hydrogen],
            'pos': np.vstack([i['coords'] for i in residues], dtype=np.float32)[hydrogen],
            'is_backbone': np.hstack([i['backbone'] for i in residues], dtype=np.bool_)[hydrogen],
            'atom_name': np.hstack([i['atom_name'] for i in residues])[hydrogen].tolist(),
            'atom_to_aa_type': np.hstack([i['atom_to_aa_type'] for i in residues], dtype=np.int64)[hydrogen],
            'res_id': np.array(sum([[idx] * len(i['element']) for idx, i in enumerate(residues)], []), dtype=np.int64)[hydrogen],
            'res_id_orig': np.array(sum([[i['orig_id']] * len(i['element']) for idx, i in enumerate(residues)], []), dtype=np.int64)[hydrogen],
        }
