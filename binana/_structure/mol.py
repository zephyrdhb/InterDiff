...
class Mol:
    ...
    def __init__(self):
        ...
        self.res_type = AA_NAME_SYM
    def load_pdb_from_known(
        self,
        text_content,
        ...
        for t in range(len(text_content['protein_element'])):
            temp_atom = Atom()
            temp_atom.atom_name = text_content['protein_atom_name'][t]
            temp_atom.resid = text_content['protein_res_id'][t] + 1

            temp_atom.coordinates = Point(
                text_content['protein_pos'][t,0].item(),
                text_content['protein_pos'][t,1].item(),
                text_content['protein_pos'][t,2].item()
            )
            temp_atom.atom_type = self.ptable.GetElementSymbol(text_content['protein_element'][t].item())
            temp_atom.element = temp_atom.atom_type
            temp_atom.residue = self.res_type[text_content['protein_atom_to_aa_type'][t].item()]
            temp_atom.charge = 0.0
            temp_atom.pdb_index = str(t)
            temp_atom.chain = 'A'
            temp_atom.atom_id = str(t)
