...
def from_known(ligand_name, receptor_data, max_cutoff=None):

    ligand = _Mol()
    ligand.load_pdb_file(ligand_name)

    receptor = _Mol()
    if max_cutoff is None:
        # Load the full receptor (all atoms).
        receptor.load_from_known(receptor_data)
    else:
        receptor.load_from_known(
            receptor,
            ligand.min_x - max_cutoff,
            ligand.max_x + max_cutoff,
            ligand.min_y - max_cutoff,
